...
public class OperationBuilderFactory {
    ...
    public static class Builder {
        ...
        public QueryBuilder builder() {
            ...
            List<QueryBuilder> mustBuilders = queryBuilderMap.get(OperationType.MUST);
            if (!CollectionUtils.isEmpty(mustBuilders)) {
                for (QueryBuilder queryBuilder : mustBuilders) {
                    boolQueryBuilder.must(queryBuilder);
                }
            }
            List<QueryBuilder> mustNotBuilders = queryBuilderMap.get(OperationType.MUST_NOT);
            if (!CollectionUtils.isEmpty(mustNotBuilders)) {
                for (QueryBuilder queryBuilder : mustNotBuilders) {
                    boolQueryBuilder.mustNot(queryBuilder);
                }
            }
            List<QueryBuilder> shouldBuilders = queryBuilderMap.get(OperationType.SHOULD);
